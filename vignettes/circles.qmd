...
```{r}
bp + 
  {c2 <- circle(point(-2, 0), radius = 1)} + 
  place(circle(radius = 1.5), c2, where = "right", sep = 1)
```
...
Multiple circles can be created at once with named positions:
...
This works the same as placing circles next to each other.
