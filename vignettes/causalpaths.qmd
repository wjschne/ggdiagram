...
ggdiagram(base_family = my_font, base_size = 16) +
...
ggdiagram(base_family = my_font, base_size = 16) +
  ...
  {B <- that |> 
    place(that, "below")} +
  ob_covariance(A, B, label = ob_label("Correlates with", vjust = 0)) 
...
ggdiagram(base_family = my_font, base_size = 16) +
  ...
  direct(e, Y) +
  {sigma_x <- ob_variance(X, 
                       bend = -20, 
                       where = degree("west"))} +
  {sigma_e <- ob_variance(e, 
                       looseness = 1.8,
                       theta = degree(60),
                       where = "east")} +
  {i <- ob_polygon(
    p = ob_polar(theta = degree(60), distance(X@center,Yhat@center)) + 
      ob_point(0,-.2) +
...
lb_error <- redefault(ob_label, size = 18, color = lb_color)
...
ggdiagram(base_family = my_font, base_size = 16) + 
  ...
  covariance(x2@point_at("west"), 
             x1@point_at("west"),
             label = lb_path("*r*~12~", position = .5))
```

The model-implied correlations among the four variables is the sum of the direct, indirect (i.e. mediated), spurious (i.e., due to common casuses), and unanalyzed effects (i.e., involving unmodeled correlations):

$$
\begin{align}
\color{gray}{r_{12}}\\
r_{13} &= \color{orchid}{b_{13}}+\color{gray}{r_{12}b_{23}}\\
r_{14} &= \color{orchid}{b_{14}}+\color{forestgreen}{b_{13}b_{34}}+\color{gray}{r_{12}b_{24}}+\color{gray}{r_{12}b_{23}b_{34}}\\
r_{23} &= \color{orchid}{b_{23}}+\color{gray}{r_{12}b_{13}}\\
r_{24} &= \color{orchid}{b_{24}}+\color{forestgreen}{b_{23}b_{34}}+\color{gray}{r_{12}b_{14}}+\color{gray}{r_{12}b_{13}b_{34}}\\
r_{34} &= \color{orchid}{b_{34}}+\color{dodgerblue}{b_{23}b_{24}}+\color{dodgerblue}{b_{13}b_{14}}+\color{gray}{b_{13}r_{12}b_{24}}+\color{gray}{b_{13}r_{12}b_{23}b_{34}}\\
\text{Key: }&\color{orchid}{\text{Direct effect}},\, \color{forestgreen}{\text{Indirect effect}},\,\color{dodgerblue}{\text{Spurious effect}},\,\color{gray}{\text{Unanalyzed effect}}
\end{align}
$$
...
ggdiagram(base_family = my_font) +
  # observed variables
  {o <- ob_ellipse(m1 = 10, a = .6, color = NA) %>%
      shape_array(5, sep = .2, fill = class_color(my_fills[2])@lighten(seq(.6, .9, length.out = 5)))} +
...
ggdiagram(base_family = my_font, base_size = 16) +
  ...
  ob_label("Loadings", 
           lb_indicator@p %|-% midpoint(l1, o3@bounding_box)) + 
  ob_label("Residuals", 
           lb_indicator@p %|-% e3@bounding_box@center) + 
  ob_label("Latent<br>Variable", lb_indicator@p %|-% l1) + 
  ob_label("Latent<br>Variance", 
           lb_indicator@p %|-% l1@normal_at(
             "north", 
             distance = .5)) + 
  ob_label("Residual<br>Variances", 
           lb_indicator@p %|-% e3[1]@normal_at(
             "south", 
             distance = .5))
...
ggdiagram(base_size = 16, base_family = my_font) +
...
```{r}
#| label: fig-wedge
#| fig-cap: Latent variable path diagram will all variables scaled to their variance sizes. Pie wedges and stacked bars display variance proportions of source variables.
#| code-fold: true
#| fig-width: 8
#| fig-height: 5.5

ggdiagram(base_family = my_font, base_size = 13) +
  ...
  {map(1:3, \(i) {
    ...
    l_b_p <- lb_path(
      round_probability(broad2indicator[[b_text]], 
                        phantom_text = "."), 
      p = b_p@line@point_at_y(b_p[2]@midpoint(.46)@y),
      color = my_fills[i])
    ...
    list(r_g, r_broad, r_unique, b_p, l_b_p, error, error2observed, lb_o) %>% lapply(as.geom)
  })}
```
...
ggdiagram(base_family = my_font, base_size = 16) +
  ...
  {Gc_3 <- ob_ellipse(a = .5,
                   ...
                   m1 = 10,
                   fill = "dodgerblue4") |> 
    place(from = Gc, where = "below", sep = 1.2) |> 
    shape_array(
      k = 3,
      where = 0,
      sep = .1,
...
ggdiagram(base_family = my_serif_font, base_size = 16) +
  {g <- ob_circle(
      radius = 1.33,
      fill = NA,
      color = NA
  ...
  ob_label(
    ...
    p = narrow[seq(nchar(str_narrow)) + 2]@center,
  ...
  ob_label(
    ...
    p = narrow[seq(nchar(str_narrow)) + 22]@center,
  ...
  {purrr::map(65 + c(0,15, 30, 45), \(x) {
    ob_label(
      ...
      p = tests[seq(nchar(str_tests)) + x]@center,
  ...
  {purrr::map(63 + c(0,15, 30, 45), \(x) {
    ob_label(
      ...
      p = specifics[seq(nchar(str_specifics)) + x]@center,
...
@curranBuildingPathDiagrams2007 developed a set of extensions to path diagrams to incorporate random variables. Their core innovation is to treat random variables as latent variables that sit atop direct paths. @fig-multilevel displays the same information as the equations listed previously, but in terms that are easier to understand and use. 
...
clr <-   class_color(c("#976A85","#7385A1","#4E9C9B","#66AB7A","#A4AF5A"))@darken(.1)@color
...
ggdiagram(base_family = my_font, base_size = 13) + 
  ...
  ob_latex(r"(\text{\emph{σ}}^{\text{2}}_{\mkern-1.5mu\text{\emph{e}}})", e_var@midpoint(), width = .035, family = "Roboto Condensed", filename = "sigma_e_2", color = e@fill)
