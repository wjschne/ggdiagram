...
The `@point_at` property of an `ellipse` object is a function that can find a point at a specific angle.
...
bp + 
  {c1 = circle(radius = 3.6, color = "gray30")} +
  {e1 <- ellipse(a = 2.8)} +
  {p1 <- c1@point_at(theta)} +
  label(theta, p1, polar_just = polar(theta, r = 1.5)) +
  segment(point(), p1, linewidth = .2) +
  {p2 <- e1@point_at(theta, definitional = T, color = "dodgerblue")} +
  segment(point(), p2) + 
  label(theta@degree, p2, polar_just = polar(theta, r = 1.5)) +
  theme_void()
```
...
```{r definitional}
ellipse(a = 2)@point_at(degree(60), definitional = TRUE)
```
...
e1 <- ellipse(point(runif(n,-2,2), 
                      runif(n,-2,2)),
                a = runif(n, .5, 1.5),
                b = runif(n, .5, 1.5),
                m1 = runif(n,2,10),
                angle = degree(runif(n, 0, 360)))
