...
```{r}
point(1:5,2, color = "blue", shape = 1:5)@tibble
```
...
p1 <- c1@center + polar(r = c1@radius * 1, theta = degree(115))
...
bp +
  c1 +
  pts@label(paste0("p", seq(pts@length)), 
            vjust = 0, 
            plot_point = TRUE) +
  intersect_c1_p2 +
  resect(seg_c1_p2, unit(1,"pt")) 
  seg_c1_p2@label(label = distance(seg_c1_p2))

  intersect_c1_p3 +
  seg_c1_p3 +
  label(p = seg_c1_p3, distance(seg_c1_p3))
...
First, let's this straight. If all you need to do make a scatterplot, just make one the standard way with ggplot2's standard functions. However, if you have reason to make points with ggdiagram and then plot them, then `as.geom` function can help.
