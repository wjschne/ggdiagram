...
```{r}
ggdiagram() + 
  geom_vline(xintercept = -3:3, color = "gray60", linetype = "dotted") +
  geom_hline(yintercept = -3:3, color = "gray60", linetype = "dotted") +
  ob_segment(ob_point(c(-3, 0), c(0, -3)), 
             {xy <- ob_point(c(3, 0), c(0, 3))}, 
             arrow_head = arrowhead(), arrow_fins = arrowhead()) +
  ob_label(c("x", "y"),
           xy,
           vjust = c(0.5, 0),
           hjust = c(0, 0.5)) +
  tibble(dx = c(-3,1,1,2, 1, 1),
         dy = c(-1, 0, 2, 0, -2, 0),
         x = cumsum(dx),
         y = cumsum(dy)) %>% 
  ob_point() %>% 
