...
hABC <- sABC@hatch(linewidth = 0.75)
...
p_orient <- bind(
  c(pG, pA, pE, pB, pA, pB, pC, pA, pC, pC, nudge(pK, -0.01, -0.01))
)
...
ggdiagram(font_family = "Roboto Condensed", font_size = 20) +
  ...
  bind(c(waiting@center, s4@center, s2@center + ob_polar(degree(c(
    90, 210, 330
  )), 0.2))) %>%
  set_props(
    color = black,
  ...
  ob_bezier(
    c(bind(
      c(waiting@west, waiting@center %-|% enter@center, enter@north)
    ), bind(
      c(s4@west, s4@center %-|% e2@center, e2@south)
    ), bind(
      c(l2@south, l2@center %|-% s4@center, s4@east)
    ), bind(
      c(leave@north, leave@center %|-% waiting@center, waiting@east)
    )),
    resect = 2,
...
```{r}
p_quadrilateral <- ob_point(c(0, 2, 1.9, 0.3), c(0, 1.2, -0.1, 1))
p_complex <- p_quadrilateral + ob_polar(degree(340), 5)
p_simple <- ob_point(c(0, 2, 1.5, 1.8), c(0, -0.1, 0.6, 1.4))
p_simple <- p_simple - p_simple@centroid + rotate(p_complex@centroid,
                                                  theta = degree(-140),
                                                  origin = p_quadrilateral@centroid)

my_polygon <- redefault(ob_polygon,
                        fill = class_color("blue")@lighten(0.1),
                        color = "blue4")
...
ggdiagram(font_family = "Roboto Condensed", font_size = 16) +
  ...
  ob_wedge(end = 60, fill = class_color("blue")@lighten(0.2)) +
  ob_arc(end = 60) +
  ...
  ob_segment(ob_point(my_ticks, -0.05), ob_point(my_ticks, 0.05)) +
  ob_segment(ob_point(-0.05, my_ticks), ob_point(0.05, my_ticks))
```
...
ggdiagram() +
  ...
  ob_point() %>%
  ob_path(color = "blue", linewidth = 1)
```

```{r}
my_fill <- grid::radialGradient(c("gold", "white")) 
```
