...
ggplot() +
  ...
  place(ob_point(mu + sigma * 0.7, dnorm(mu + sigma * 0.7, mu, sigma)), 
        where = "right", 
        sep = 3) + 
...
If you want the best image quality possible for a LaTeX snippet in a ggplot2 plot, then give the [xdvir](https://cran.r-project.org/package=xdvir) package a try. It can put LaTeX in plot titles, subtitles, and captions, as well as anywhere on the plot. It gets the typography right as well. Be warned! At least for now, xdvir renders S L O W L Y.
