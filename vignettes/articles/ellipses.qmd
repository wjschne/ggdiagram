...
As seen in @fig-ellipseroated, rotate an ellipse by setting the `angle` property with a `degree`, `radian`, or `turn` object or a numeric value (degrees).
...
Setting the angle of an ellipse will rotate it about its center. If you want to rotate it around a different point, use the `rotate` function.
