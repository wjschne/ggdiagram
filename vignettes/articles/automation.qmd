...
The `bind` function will take a list of the ggdiagram objects and a create single object. If all objects in the list are of a single type (e.g., `ob_point`), `bind` will return an object of that type. If the objects are of multiple types, `bind` will bind each type of object separately and return an `ob_shape_list`.
...
The `unbind` function is needed when we want to loop through each element using `lapply` or `purrr::map`. In @fig-unbind, we have three points arranged in an equilateral triangle. We `unbind` the points so that we can use `map` to place three points around each point. Each point is then plotted as triangles. To make the list something that ggplot2 can plot, we can `bind` the list into a single object.
...
ggdiagram() + 
  ...
  bind() +
  scale_x_continuous(NULL, expand = expansion(0.15))
```

Alternatively, we can convert each list item into a geom with `as.geom` as in @fig-unbindgeom.
...
In a more practical example, every variable in the left column of @fig-bindt1t2 is connected to every variable on the right.

```{r}
#| label: fig-bindt1t2
#| fig-cap: Using `map_ob` to the connect variables
k <- 5
clr <- viridis::viridis(k, begin = 0.3, end = 0.7)
ggdiagram() +
...
theta <- degree(seq(0,330,30))
clr <- hsv(theta@degree / 360, s = 0.4, v = 0.6)
ggdiagram() +
  {c12 <- ob_circle(
...
latent_colors <- l_names %>% 
  length() %>% 
  viridis::viridis(alpha = 0.5, begin = 0.2, end = 0.75) %>% 
  `names<-`(rev(l_names))
...
e <- d %>% 
  ...
  pmap(\(id, data) {
    ...
    connect(from = s[id], 
            ...
            label = ob_label(ll, angle = 0, position = 0.47), 
            label_sloped = F,
            resect = 1, 
