...
ggdiagram(font_size = 16, font_family = my_font) +
  ...
  D@west@label("D", hjust = 1.1, fill = NA) +
  E@east@label("E", hjust = -0.1, fill = NA) +
  C@label("C", vjust = -0.2, hjust = 0.6, fill = NA) +
  Cprime@label("C&prime;", vjust = 1.3, fill = NA) +
  CCprime + 
  pF +
  pF@label("F", vjust = 1.1, hjust = -0.3, fill = NA)
...
rvector <- function(scalar = 1) {
  scalar * ob_point(
    runif(n = 1, min = -0.1, max = 0.1),
    runif(n = 1, min = -0.1, max = 0.1)
  )
}

ggdiagram(font_size = 16, font_family = my_font) +
  {A <- ob_point(0, 0) + rvector(0.1)} +
  {B <- ob_point(1.25, 0.25) + rvector(0.1)} +
  {AB <- ob_segment(A, B, color = blue)} +
  ...
  ob_polygon(p = {p_ABC <- bind(c(A, B, C))}, 
             color = NA, 
             fill = class_color("orange")@lighten(0.3)) +
...
ggdiagram(font_family = my_font) +
  ...
  {E <- AE@point_at_y(L@y - 0.5)} +
  {`F` <- BF@point_at_y(G@y - 0.5)} +
  ...
  scale_y_continuous(NULL, breaks = NULL, minor_breaks = NULL, expand = expansion(c(0.1, 0.01)))
```
