...
format:
  html:
    ...
    fig-height: 7
vignette: >
  %\VignetteIndexEntry{angles} %\VignetteEngine{quarto::html}
...
Angles have different kinds of units associated with them: turns (1 turn = one
full rotation a circle), degrees (1 turn = 360 degrees), and radians (1 turn =
$2\pi$ = $\tau$).

I like &pi; just fine, but I agree with Michael Hartl's [Tau
Manifesto](https://tauday.com/tau-manifesto) that we would have been better off
if we had recognized that the number of radians to complete a full turn of a
circle (&tau; = 2&pi; &asymp; 6.283185) is more fundamental than the number of
radians to complete a half turn (&pi;).
...
r <- seq(1, 0.5, length.out = length(angle_types))
...
ggplot() +
  ...
  ob_segment(ob_point(), p, linewidth = 0.25) +
  purrr::pmap(
...
as.character(turn(0.25))
as.character(radian(0.5 * pi))
```
...
Angles can be added, subtracted, multiplied, and divided. The underlying value
stored can be any real number (in turn units), but degrees, radians, and turns
are always displayed as between &minus;1 and +1 turns, &pm;360 degrees, or
&pm;2&pi; radians.
...
You can ensure that an angle is always positive or negative with the `@positive`
...
The outputs of `degree`, `radian`, and `turn` can take the three standard
...
# my_colors <- purrr::map2_chr(
#   scico::scico(6, palette = "hawaii"),
#   c(0.01, 0.01, 0.01, 0.01, 0.15, 0.4),
#   tinter::darken
# )
...
Benefits of using trigonometric functions with angles instead of numeric radians
include:

- Angle metric conversions are handled automatically.
- Under the hood, the `cospi`, `sinpi`, and `tanpi` functions are used to get
  the rounding right on key locations (e.g., 90 degrees, 180 degrees)
...
Angles created with the `degree`, `radian`, or `turn` function are `ob_angle`
objects. The `ob_angle` function exists but is not meant to be used directly.
Its underlying data is a vector of numeric data representing the number of
turns. The underlying turn data from any `ob_angle` object can be extracted with
the `c` function (or with the `S7::S7_data` function).
