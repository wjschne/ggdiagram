...
ggdiagram(font_family = my_font, font_size = 16) +
  ...
  {i <- ob_circle(n = 3, 
    ...
    place(r, "right", 0.3) + ob_point(0, -0.25)} 
...
error <- redefault(ob_circle, color = ln_color, radius = 0.5)
...
ggdiagram(font_family = my_font, font_size = 16) +
  ...
  {Y <- observed(
    label = lb_observed("*Y*")) |> 
    place(Yhat, "right", sep = 0.75)} +
  # Error
  {e <- error(label = lb_observed("*e*", nudge_y = 0)) |> 
    place(Y, "right", sep = 0.75)} +
...
ggdiagram(font_family = my_font, font_size = 16) + 
  ...
  covariance(x2@west, 
             x1@west,
             label = lb_direct("*r*~12~", position = 0.5))
```
...
my_fills <- viridis::viridis(n = 3, begin = 0.3, end = 0.6) |> 
  class_color() |> 
  set_props(saturation = 0.6, brightness = 0.5) |> 
  c()
...
ggdiagram(font_family = my_font) +
  # observed variables
  {o <- ob_ellipse(m1 = 10, a = 0.6, color = NA) |>
      ob_array(5, sep = 0.2, fill = class_color(my_fills[2])@lighten(seq(0.6, 0.9, length.out = 5)))} +
  ...
  ob_label("Formative<br>Indicators", size = 16, fill = NA) |> 
    place(o@bounding_box@east, sep = 0.8) + 
...
my_resect <- 1
broad <- c("Gv", "Gf", "Gc")
g2broad <- c(Gv = 0.84, Gf = 0.95, Gc = 0.80)

broad2indicator <- list(Gv = c(0.78, 0.84, 0.91),
                        Gf = c(0.88, 0.81, 0.74),
                        Gc = c(0.74, 0.91, 0.93))
...
ggdiagram(font_family = my_font, font_size = 16) +
  ...
  {vv <- map_ob(Gx, \(b) {
    ...
    o <- ob_array(
      ...
      k = 3, 
      sep = 0.2,
      fill = purrr::map_chr(c(0.6, 0.75, 0.9), tinter::lighten, x = o1@fill),
      label = lb_observed(
        paste0(
          b@label@label, 
          "~", 
    ...
    l <- lb_direct(
      round_probability(
        ...
        phantom_text = "."), 
      center = p@line@point_at_y(p[2]@midpoint(position = 0.47)@y),
      color = b@fill)
...
ggdiagram(font_family = my_font, font_size = 13) +
  ...
  ob_wedge(center = Gx@center, 
           radius = sqrt(1 / pi),
           start = turn(0.5) + turn(broad_variance) / 2,
           end = turn(0.5) - turn(broad_variance) / 2,
           fill = my_fills) + 
  lb_latent(broad, center = Gx@center, vjust = 0.6) +
  {map(1:3, \(i) {
    b <- Gx[i]
    ...
    error <- ob_circle(r_unique@south - ob_point(0, r + 0.45), radius = r, color = NA, fill = error_color)
...
ggdiagram(font_family = my_serif_font, font_size = 16) +
  ...
  purrr::map(unbind(broad), \(b) {
    geom_polygon(
      ...
      fill = grid::radialGradient(
        colours = c(tinter::lighten(b@fill, 0.4), 
                    tinter::lighten(b@fill, 0.8)), 
        stops = c(0,1),
        group = FALSE))}) +
  ob_label(
    c("Ga", "Gc", "Gf", "Gv", "Gs", "Gr", "Gl", "Gwm"),
    broad@center + ob_point(0, -0.05),
    fill = NA,
    family = my_serif_font,
    size = 20,
    vjust = 0.55,
  ...
  ob_path(
    broad |> 
      set_props(radius = c(rep(0.66, 4), rep(0.59, 4))) |> 
      unbind() |> 
      purrr::map(\(x) {
  ...
  ob_path(
    broad |> 
      set_props(radius = c(rep(0.59, 4), rep(0.66, 4))) |> 
      unbind() |> 
      purrr::map(\(x) {
        x@point_at(
  ...
  {purrr::imap(unbind(broad), \(x, idx) {
    connect(x, narrow[(idx - 1) * 5 + 1:5], 
            color = tinter::lighten(x@fill, 0.5), 
            linewidth = 0.5,
            length_head = 6,
            resect = 1) |> 
      as.geom()
  ...
  {test_theta <- degree(seq(0, 360 - 360/120, 360/120) + 360/240)
    tests <- ob_ellipse(
      ...
      angle = test_theta,
      color = NA,
      fill = rep(tinter::lighten(broad@fill, 0.2), each = 15)) |>
      place(from = g, where = test_theta, sep = 6.2)} +
  {specifics <- ob_circle(
    radius = 0.2, 
    angle = test_theta, 
    color = NA, 
    fill = rep(tinter::lighten(broad@fill, 0.15), each = 15)) |> 
  ...
  scale_fill_manual(
    values = map(broad@fill, 
                 \(fill) { 
                   grid::radialGradient(c(tinter::lighten(fill, 0.15), 
                                          tinter::lighten(fill, 0.4)), 
                                        stops = c(0.2, 1), 
                                        group = FALSE)}) |> 
      `names<-`(broad@fill)) +
...
random_slope <- redefault(ob_circle, color = NA, radius = 0.5)
tau_covariance <- redefault(ob_covariance, 
                            linewidth = 0.5, 
                            head_length = 4, 
                            resect = 1)

latent_slope_line <- ob_line(slope = -0.8, intercept = 1.3)
...
ggdiagram(font_family = my_font, font_size = 13) + 
  ...
  {e <- ob_circle(radius = 0.6, fill = y@fill, color = NA, label = lb_observed("*e~ij~*")) |> place(y, "right", 0.9)} +
  connect(e, y, color = y@fill, resect = 2) +
  {e_var <- ob_variance(e, "east", color = e@fill, looseness = 1.7)} +
  ob_latex(r"(\text{\emph{σ}}^{\text{2}}_{\mkern-1.5mu\text{\emph{e}}})", e_var@midpoint(), width = 0.45, family = "Roboto Condensed", filename = "sigma_e_2", color = e@fill)
...
my_connect <- redefault(connect, 
                        resect = 2, 
                        color = color_x@lighten(0.85))
my_connect2 <- redefault(connect,
                         length_head = 5,
                         resect = 1,
                         color = color_x@lighten(0.70))
my_error <- redefault(ob_circle,
                      radius = 0.35,
                      fill = color_x@lighten(0.7),
                      color = NA)
my_font <- "Roboto Condensed"
k <- 4
my_sep <- 2.5
...
ggdiagram(font_family = my_font, font_size = 24) +
  ...
  {y <- my_observed(label = my_label(paste0("*Y*~", 1:k, "~")), 
                    fill = color_y@lighten(0.85)) |>
    place(x, "right", sep = 7)} +
  {e_x <- my_error(radius = 0.5) |> place(x, "right", sep = e_sep)} +
  {e_y <- my_error(radius = 0.5, 
                   fill = color_y@lighten(0.70)) |> 
    place(y, "left", sep = e_sep)} +
  {ee_x <- my_error(fill = color_x@lighten(0.55)) |> place(e_x[-1], "right", sep = e_sep)} +
  {ee_y <- my_error(fill = color_y@lighten(0.55)) |> place(e_y[-1], "left", sep = e_sep)} +
  {lxx <- my_connect(e_x[-k], e_x[-1], color = color_x@lighten(0.70))} +
  ob_label("*b~XX~*", lxx@midpoint(0.47), size = 14) +
  {lxy <- my_connect(e_x[-k], e_y[-1], color = color_x@lighten(0.70))} +
  ob_label("*b~XY~*", lxy@midpoint(0.27, color = color_x@lighten(0.70)), size = 14) +
  {lyx <- my_connect(e_y[-k], e_x[-1], color = color_y)} +
  ob_label("*b~YX~*", lyx@midpoint(0.27, color = color_y@lighten(0.70)), size = 14) +
  {lyy <- my_connect(e_y[-k], e_y[-1], color = color_y@lighten(0.70))} + 
  ob_label("*b~YY~*", lyy@midpoint(0.47), size = 14) +
  my_connect2(e_x, x, color = color_x@lighten(0.70)) + 
  my_connect2(e_y, y, color = color_y@lighten(0.70)) +
  my_connect2(ee_x, e_x[-1], color = color_x@lighten(0.55)) +
  my_connect2(ee_y, e_y[-1], color = color_y@lighten(0.55)) +
  ob_covariance(e_x[1], e_y[1], color = color_xy@lighten(0.70)) + 
  ob_covariance(ee_y, ee_x, 
                resect = 1,
                length_head = 4, 
                length_fins = 4, 
                looseness = 1.3,
                color = color_xy@lighten(0.55)) +
  ...
  {ly <- my_connect(iy, y@point_at(degree(5)), color = color_y@lighten(0.85))} + 
  ob_label("1",
