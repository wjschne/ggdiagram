...
ggdiagram() +
  ...
  ob_circle(fill = my_color@transparentize(.5), center = ob_point(1,0))
```
...
```{r hsvsequence}
#| fig-height: 1.5
my_color_array <- class_color(my_color, hue = my_color@hue + seq(0,720, length.out = 12))
...
```{r samehue}
#| fig-height: 1.5
my_color_array <- class_color(my_color, saturation = seq(.1,.6,.1))
