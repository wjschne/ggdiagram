...
inside <- S7::new_generic("inside", c("x", "y"), fun = function(x, y) {
  S7::S7_dispatch()
})
...
point_in_polygon <- function(x, y, vertices) {
  ...
  if (!all(c("x", "y") %in% colnames(vertices))) {
    stop("vertices must have two columns for x and y coordinates")
  }
...
S7::method(inside, list(ob_point, ob_ngon)) <- function(x, y) {
  if (!(x@length == 1 || y@length == 1 || x@length == y@length)) {
    stop(paste0(
      "ob_point and ob_ngon with length > 1 need to be of the same size."
    ))
  }
