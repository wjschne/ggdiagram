...
S7::method(connect, list(ob_polygon, ob_polygon)) <- function(
  ...
  connect(
    intersection(from, centroid_segment),
    intersection(to, centroid_segment),
    label = label,
...
S7::method(get_tibble, ob_ngon) <- function(x) {
  ...
  d |>
    dplyr::mutate(
      ...
      theta = purrr::map2(n, angle, \(nn, aa) {
        seq(0, 360, length.out = nn + 1) + aa
      })
    ) |>
    tidyr::unnest(theta) |>
    dplyr::mutate(
      x = cospi(theta / 180) * r + x0,
  ...
  properties = list(
    ...
    arcs = S7::new_property(getter = \(self) {
      map_ob(self, \(s) {
        ntheta <- 360 / s@n
        even <- 1 - s@n %% 2
        theta <- seq(0, 360, length.out = s@n + 1)[1:s@n] +
          s@angle@degree +
          even * 180 / s@n
