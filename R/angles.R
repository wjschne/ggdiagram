...
degree <- S7::new_class(
  ...
  constructor = function(degree = numeric(0)) {
    if (is.character(degree)) {
      degree <- cardinalpoint(degree)
    }
    if (S7::S7_inherits(degree, ob_angle)) {
      degree <- c(degree) * 360
    }
...
radian <- S7::new_class(
  ...
  constructor = function(radian = numeric(0)) {
    if (is.character(radian)) {
      radian <- cardinalpoint(radian) * pi / 180
    }
    if (S7::S7_inherits(radian, ob_angle)) {
      radian <- c(radian) * 2 * pi
    }
...
turn <- S7::new_class(
  ...
  constructor = function(turn = numeric(0)) {
    if (is.character(turn)) {
      turn <- cardinalpoint(turn) / 360
    }
    if (S7::S7_inherits(turn, ob_angle)) {
      turn <- c(turn)
    }
    S7::new_object(turn)
  }
...
S7::method(str, ob_angle) <- function(
  object,
  nest.lev = 0,
  additional = FALSE,
...
S7::method(as.character, ob_angle) <- function(
  x,
  ...,
  digits = NULL,
  type = NULL
  ...
  if (!is.null(type)) {
    ...
    if (type == "radian" || type == "ggdiagram::radian") {
      x <- radian(x)
    }
  ...
  if (inherits(x, "ggdiagram::ob_angle") || inherits(x, "ggdiagram::degree")) {
    ...
    x <- paste0(
      signs::signs(x@degree, accuracy = 10^(-1 * digits), drop0trailing = TRUE),
      "\u00B0"
    )
  }
