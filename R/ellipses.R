...
el_props <- list(
  ...
  derived = list(
    ...
    style = S7::new_property(
      getter = function(self) {
        pr <- purrr::map(el_styles, prop, object = self) |>
          `names<-`(el_styles)
    ...
    normal_at = S7::new_property(
      ...
      getter = function(self) {
        \(theta = degree(0), distance = 1) {
          ...
          p1 <- ob_point(
            sign(cos(theta)) *
              abs((self@a^(-1 * self@m1)) * self@m1 * (p0@x^(self@m1 - 1))),
            sign(sin(theta)) *
              abs((self@b^(-1 * self@m2)) * self@m2 * (p0@y^(self@m2 - 1)))
          )
          self@point_at(
            theta
          ) +
        ...
        if (definitional) {
          return(
            self@center +
              rotate(
                ob_point(
                  abs(cos(theta)^(2 / self@m1)) * self@a * sign(cos(theta)),
    ...
    tangent_at = S7::new_property(
      ...
      getter = function(self) {
        \(theta = degree(0), ...) {
          ...
          l <- ob_segment(
            p1 = p,
            p2 = rotate(p_normal, degree(90), origin = p)
          )@line
    ...
    polar_line_at = S7::new_property(
      ...
      getter = function(self) {
        \(x) {
          x0 <- x - self@center
          ob_line(
            a = x0@x * self@b^self@m2,
            b = x0@y * self@a^self@m1,
            c = -1 *
              (self@b^self@m2) *
              (self@a^self@m1) -
              self@center@x * x0@x -
...
S7::method(str, ob_ellipse) <- function(
  ...
  omit = omit_props(
    object,
    include = c("center", "a", "b", "angle", "m1", "m2")
  )
) {
...
S7::method(projection, list(ob_point, centerpoint)) <- function(
  p,
  object,
  ...
) {
...
S7::method(get_tibble_defaults, ob_ellipse) <- function(x) {
  ...
  get_tibble_defaults_helper(
    x,
    sp,
    required_aes = c("x0", "y0", "a", "b", "m1", "m2", "angle")
  )
}
...
S7::method(connect, list(centerpoint, ob_point)) <- function(
  ...
  theta <- radian(atan2(to@y - from@center@y, to@x - from@center@x))
...
S7::method(connect, list(ob_point, centerpoint)) <- function(
  ...
  theta <- radian(atan2(to@center@y - from@y, to@center@x - from@x))
...
S7::method(ob_variance, centerpoint) <- function(
  ...
  if (!S7::S7_inherits(where, ob_angle)) {
    where <- degree(where)
  }
  if (!S7::S7_inherits(theta, ob_angle)) {
    theta <- degree(theta)
...
# Placing ----
S7::method(place, list(centerpoint, centerpoint)) <- function(
  x,
  from,
  where = "right",
  sep = 1
...
S7::method(place, list(ob_point, centerpoint)) <- function(
  x,
  from,
  where = "right",
  sep = 1
...
S7::method(place, list(centerpoint, ob_point)) <- function(
  x,
  from,
  where = "right",
  sep = 1
...
S7::method(place, list(ob_line, ob_ellipse)) <- function(
  x,
  from,
  where = "right",
  sep = 1
...
S7::method(ob_array, ob_ellipse) <- function(
  x,
  k = 2,
  sep = 1,
  where = "east",
