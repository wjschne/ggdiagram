...
pr_place <- S7::new_property(S7::class_function, getter = function(self) {
  \(from, where = "right", sep = 1, ...) {
    place(x = self, from = from, where = where, sep = sep, ...)
  }
})
...
pt_props <- list(
  ...
  derived = list(
    ...
    bounding_box = S7::new_property(getter = function(self) {
      ob_rectangle(
        southwest = ob_point(x = min(self@x), y = min(self@y)),
    ...
    r = S7::new_property(
      ...
      setter = function(self, value) {
        set_props(
          self,
          x = cos(self@theta) * value,
          y = sin(self@theta) * value
        )
      }
    ),
    ...
    style = S7::new_property(
      getter = function(self) {
        pr <- purrr::map(pt_styles, prop, object = self) |>
          `names<-`(pt_styles)
...
S7::method(polar2just, S7::class_numeric) <- function(
  x,
  multiplier = NULL,
  axis = c("h", "v")
) {
...
S7::method(polar2just, S7::class_character) <- function(
  x,
  multiplier = NULL,
  axis = c("h", "v")
) {
...
S7::method(polar2just, ob_angle) <- function(
  x,
  multiplier = NULL,
  axis = c("h", "v")
) {
...
S7::method(polar2just, ob_point) <- function(
  x,
  multiplier = NULL,
  axis = c("h", "v")
) {
...
S7::method(polar2just, ob_polar) <- function(
  x,
  multiplier = NULL,
  axis = c("h", "v")
) {
...
#' @export
#' @returns ob_point object
`%|-%` <- S7::new_generic("%|-%", c("e1", "e2"), fun = function(e1, e2) {
  S7::S7_dispatch()
})
...
S7::method(connect, list(ob_point, ob_point)) <- function(
  ...
  if (
    is.null(from_offset) &&
      is.null(to_offset) &&
      (is.null(arc_bend) || all(arc_bend == 0))
  ) {
    s <- ob_segment(
      from,
      to,
    ...
    theta_end[arc_bend < 0 & theta_end > theta_start] <- theta_end[
    ...
    if (is.character(from_offset)) {
      from_offset <- ob_polar(
        theta = degree(from_offset),
        r = distance(from, to)
      )
    }
    ...
    if (!is.null(from_offset)) {
      ...
      d <- d |>
        dplyr::mutate(fromoffset_x = from1@x, fromoffset_y = from1@y)
    }

    if (!is.null(to_offset)) {
      to1 <- to + to_offset
      d <- d |>
        dplyr::mutate(tooffset_x = to1@x, tooffset_y = to1@y)
    }
...
S7::method(place, list(ob_point, ob_point)) <- function(
  x,
  from,
  where = "right",
  sep = 1
...
S7::method(
  nudge,
  list(ob_point, S7::class_numeric, S7::class_numeric)
) <- function(object, x, y) {
  object + ob_point(x, y)
}
...
S7::method(
  nudge,
  list(ob_point, S7::class_numeric, S7::class_missing)
) <- function(object, x, y) {
  object + ob_point(x, 0)
}
...
S7::method(
  nudge,
  list(ob_point, S7::class_missing, S7::class_numeric)
) <- function(object, x, y) {
  object + ob_point(0, y)
}

S7::method(ob_array, ob_point) <- function(
  x,
  k = 2,
  sep = 1,
  where = "east",
...
S7::method(ob_covariance, list(ob_point, ob_point)) <- function(
  ...
  if (!S7::S7_inherits(where, ob_angle) && !is.null(where)) {
    where <- degree(where)
  }
  if (!S7::S7_inherits(bend, ob_angle)) {
    bend <- degree(bend)
  }

  p <- purrr::pmap(
    list(xx = unbind(x), yy = unbind(y), bb = unbind(bend)),
    \(xx, yy, bb) {
      if (is.null(where)) {
        d_xy <- yy - xx
  ...
  if (!is.null(dots$label)) {
    l <- dots$label
    if (!S7::S7_inherits(l, ob_label)) {
      l <- ob_label(l)
    }
    dots$label <- NULL
  }

  rlang::inject(ob_bezier(
    p = p,
    label = l,
    label_sloped = FALSE,
    arrow_head = arrow_head,
