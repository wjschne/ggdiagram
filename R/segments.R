...
sg_props <- list(
  ...
  derived = list(
    distance = new_property(getter = function(self) {
      distance(self)
    }),
    length = new_property(
...
stroke_width = self@stroke_width
  ...
  funs = list(    
    label = new_property(class_function, 
      getter = function(self) {
        \(label, position = .5, vjust = 0, angle = self@line@angle, ...) {
        label(p = midpoint(self, position = position), 
              label = label, 
              vjust = vjust, 
              angle = angle, 
              ...)
...
segment <- new_class(
  ...
  constructor = function(p1 = class_missing,
    ...
    s_style <- p1@style + p2@style + style + style(
      alpha = alpha,
...
stroke_width = stroke_width
    ) + rlang::inject(style(!!!user_overrides))
...
method(resect, 
       list(segment,
            class_numeric_or_unit)) <- function(x,distance, distance_end = distance) {
# d <- x@p2 - x@p1
# x@p1 <- x@p1 + polar(theta = d@theta, r = distance)
# x@p2 <- x@p2 + polar(theta = d@theta + turn(.5), r = distance_end)
#               print(x)
#          x
              # x <- segment(point(), point(1,1))
              x@resect_head <- distance
              x@resect_fins <- distance_end
              x
