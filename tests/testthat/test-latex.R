...
test_that("ob_latex: basic construction produces correct S7 class", {
  ...
  expect_no_error(ggplot2::update_ggplot(lx2, ggdiagram())) # update
  lx3 <- ob_latex(
    "x",
    height = 2,
    math_mode = FALSE,
    ...
    angle = 45,
    border = c(2, 3, 4, 5),
    fill = class_color("grey80"),
    latex_packages = "bm",
    filename = fn
  )
  expect_no_error(as.geom(lx3))
  expect_false(lx3@math_mode) # math_mode = FALSE omits dollar signs from tex
  expect_equal(lx3@height, 2) # height-based sizing respected when width is omitted
  ...
  lx <- lx_base
  lx@rectangle <- ob_rectangle(
    center = ob_point(5, 5),
    width = 3,
    height = 2,
    angle = degree(30)
  ) # @rectangle setter updates center, width, height, angle
  ...
  lx <- lx_base
  expect_error(lx@rectangle <- ob_point(1, 1),
               "must be the output of the function `ob_rectangle`") # @rectangle setter errors on non-ob_rectangle input
