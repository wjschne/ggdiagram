...
test_that(desc = "class_margin", {
  expect_identical(
    ggdiagram:::class_margin(list(grid::unit(1, "pt"))),
    list(ggdiagram:::class_margin(grid::unit(1, "pt")))
  )
  expect_identical(
    ggdiagram:::class_margin(1),
    ggdiagram:::class_margin(grid::unit(1, "pt"))
  )
  expect_identical(
    ggdiagram:::class_margin(3),
    ggdiagram:::class_margin(ggplot2::margin(3, 3, 3, 3))
  )
  expect_identical(
    ggdiagram:::class_margin(c(1, 2, 1, 2)),
    class_margin(grid::unit(1:2, "pt"))
  )
  expect_identical(
    ggdiagram:::class_margin(c(1, 2, 3, 4)),
    ggdiagram:::class_margin(grid::unit(1:4, "pt"))
  )
  expect_error(
    ggdiagram:::class_margin(grid::unit(1:5, "pt")),
    "Margins can have 1 \\(all sides\\), 2 \\(horizontal vs vertical)\\, or 4 \\(top right bottom left\\) elements\\."
  )
  expect_error(
    ggdiagram:::class_margin("3"),
    "Margins can be of class margin, unit, or numeric"
  )
  expect_length(
    ggdiagram:::class_margin(list(
      ggdiagram:::class_margin(1),
      ggdiagram:::class_margin(1)
    )),
    2L
...
test_that(desc = "class_arrowhead", {
  ...
  expect_identical(
    ggdiagram:::class_arrowhead(class_arrowhead(m1)),
    ggdiagram:::class_arrowhead(m1)
  )
  expect_identical(
    ggdiagram:::class_arrowhead(list(m1, m2)),
    list(ggdiagram:::class_arrowhead(m1), class_arrowhead(m2))
  )
  expect_error(
    ggdiagram:::class_arrowhead(1),
    "Arrowheads must be a 2-column matrix of numbers\\."
  )
  expect_error(
    ggdiagram:::class_arrowhead(matrix(1)),
    "Arrowheads must be a 2-column matrix of numbers\\."
  )
  expect_no_error(set_default_arrowhead())
  expect_no_error(set_default_arrowhead(matrix(
    c(1, 0, 0, 1),
    nrow = 2,
    byrow = TRUE
  )))
...
test_that("str no error", {
  ...
  expect_no_error(suppressMessages(capture.output(str(ob_segment(ob_point(
    0,
    1:3
  ))))))
  expect_no_error(suppressMessages(capture.output(str(ob_arc(
    start = 0,
    end = 90,
    radius = 1,
    label = "A"
  )))))
  expect_no_error(suppressMessages(capture.output(str(ob_bezier(ob_point(
    c(0, 1, 2),
    c(0, 2, 0)
  ))))))
  expect_no_error(suppressMessages(capture.output(str(ob_path(ob_point(
    c(0, 1, 2),
    c(0, 2, 0)
  ))))))
  expect_no_error(suppressMessages(capture.output(str(ob_polygon(ob_point(
    c(0, 1, 2),
    c(0, 2, 0)
  ))))))
...
test_that("ob_shape_list", {
  ...
  expect_length(
    bind(ob_shape_list(list(ob_point(), ob_point(1, 1), ob_circle()))),
    2L
  )
  ...
  expect_true(
    S7::S7_inherits(bind(ob_shape_list(list(ob_point(), ob_point())))),
    ob_point
  )
...
test_that("ob_array", {
  expect_error(
    ob_array(ob_circle(x = 0, y = 1:2), k = 3),
    "The shape must start with an object of length 1."
  )
  expect_no_error(ob_array(
    ob_circle(x = 0, y = 1, label = "a"),
    k = 3,
    anchor = "south"
  ))
  expect_no_error(ob_array(
...
test_that("helpers", {
  ...
  expect_identical(
    ggdiagram:::get_tibble_defaults_helper(
      ...
      default_style = ob_style(alpha = 1)
    )[, "alpha", drop = TRUE],
    c(0.5, 1.0)
  )
  ...
  expect_identical(
    round_probability(0, digits = 2, max_digits = 3, round_zero_one = FALSE),
    ".000"
  )
  expect_identical(
    round_probability(1, phantom_text = "+"),
...
test_that("lead and lag cycle", {
  ...
  expect_error(
    lead_cycle(1:2, n = 2),
    "n must be a positive integer less than length of x."
  )
  expect_error(
    lag_cycle(1:2, n = 2),
    "n must be a positive integer less than length of x."
